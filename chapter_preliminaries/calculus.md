...
簡単に言えば、*微分*は、引数の変化に対する関数の変化率です。デリバティブは、各パラメータを無限に少しだけ*増加*または*減少*した場合、損失関数がどれだけ速く増加または減少するかを教えてくれます。正式には、スカラーからスカラーにマップする関数$f: \mathbb{R} \rightarrow \mathbb{R}$の場合、[**$f$の*微分* $x$はとして定義されます**] 
